...
function exportSavedAudit(id) {
  var rec = getSaved().find(function(a) { return a.id === id; });
  if (!rec) { toast('Not found'); return; }
  var name = (rec.customer.name || 'audit').replace(/[^a-zA-Z0-9]/g,'-');
  var date = rec.customer.date || new Date().toISOString().split('T')[0];
  dlJSON(rec, date + '_' + name + '.json');
  toast('Exported: ' + name);
}
...
function exportAll() {
  ...
  var bundle = { exportedAt: new Date().toISOString(), source: 'AuditFieldTool', auditCount: saved.length, audits: saved };
  dlJSON(bundle, new Date().toISOString().split('T')[0] + '_all-audits.json');
  toast('Exported ' + saved.length + ' audits');
}
  ...
  setTimeout(function() {
    try {
      ...
      S.photos.forEach(function(photo, index) {
        ...
        doc.setFontSize(8);
        doc.setFont('helvetica', 'bold');
        doc.text('PHOTO ' + (index + 1) + ' OF ' + photoCount, margin, yPos);
        yPos += 5;
        ...
        try {
          var imgData = photo.dataUrl;
          var imgProps = doc.getImageProperties(imgData);
          var imgW = contentW;
          var imgH = (imgProps.height * imgW) / imgProps.width;
          var maxImgH = 140;
          if (imgH > maxImgH) {
            imgW = (maxImgH * imgProps.width) / imgProps.height;
            imgH = maxImgH;
          }
          doc.addImage(imgData, 'JPEG', margin, yPos, imgW, imgH, '', 'MEDIUM');
          yPos += imgH + 4;
        } catch(imgErr) {
        ...
        if (photo.note) {
          ...
          doc.roundedRect(margin, yPos, contentW, noteH, 2, 2, 'FD');
          doc.setTextColor(204, 204, 204);
          doc.setFontSize(8);
          doc.setFont('helvetica', 'normal');
          doc.text(noteLines, margin + 4, yPos + 5);
          yPos += noteH + 6;
        } else {
          doc.setTextColor(100, 100, 100);
          doc.setFontSize(7);
          doc.setFont('helvetica', 'italic');
          doc.text('No note attached', margin, yPos + 3);
          yPos += 8;
        }

        if (index < S.photos.length - 1) {
          doc.setDrawColor(51, 51, 51);
          doc.line(margin, yPos, pageW - margin, yPos);
          yPos += 8;
        }

        if (yPos > pageH - 40 && index < S.photos.length - 1) {
          doc.addPage();
          yPos = margin;
        }
      });
...
function renderWeeklyBatches() {
  ...
  weeks.forEach(function(week) {
    ...
    group.querySelector('.week-json-btn').addEventListener('click', function() {
      ...
      toast('Exported ' + week.audits.length + ' audits for ' + week.label);
    });
...
function exportSavedPhotoPDF(audit, callback) {
  ...
  try {
    ...
    photos.forEach(function(photo, index) {
      if (index > 0 && yPos > pageH - 80) { doc.addPage(); yPos = margin; }
      ...
      try {
        ...
        var imgH = (imgProps.height * imgW) / imgProps.width;
        var maxImgH = 140;
        if (imgH > maxImgH) { imgW = (maxImgH * imgProps.width) / imgProps.height; imgH = maxImgH; }
        doc.addImage(photo.dataUrl, 'JPEG', margin, yPos, imgW, imgH, '', 'MEDIUM');
        yPos += imgH + 4;
      } catch(e) {
        doc.setTextColor(255,100,100);
        doc.setFontSize(8);
      ...
      if (yPos > pageH - 40 && index < photos.length - 1) { doc.addPage(); yPos = margin; }
    });
